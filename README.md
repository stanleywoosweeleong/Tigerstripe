...
- Each slide with an original photo has Save / Share buttons: the photo is exported as a JPEG with a green footer stamped “Slide NN / 45 · Prof. Dr. Tran Van Hau · Can Tho University · AGRI Malaysia 2026” plus the app URL, so forwarded pictures keep their number and credit. Share uses the phone's share sheet (WhatsApp etc.); on a desktop browser without file sharing it copies the photo to the clipboard and opens WhatsApp with the caption (Ctrl+V pastes the photo); if the clipboard refuses it saves the file instead. The ☰ drawer has “Download all original slides (ZIP)” — 40 numbered JPEGs + README, built in the browser (works offline), about 4 MB.
- Language toggle cycles EN / 中文 / BM / VI / KM (first launch shows a language picker; the choice is remembered, and the header button changes it any time); Malay text uses Belang Harimau, Vietnamese Sọc hổ, Khmer ឆ្នូតខ្លា for the disorder. Vietnamese and Khmer were added at Prof. Tran's request (12 Sep 2026); they are machine-assisted translations keyed to the English text — corrections welcome. Terminology: Vietnamese follows Prof. Tran's own usage — the disorder family is *sượng cơm*, and the tiger-stripe type is glossed as flesh with uneven yellow/white colour "như 'da lợn'"; the app calls it *sượng sọc hổ*. No established Khmer term was found, so *ឆ្នូតខ្លា* is a literal rendering glossed as hard, unevenly coloured flesh — Cambodian readers are invited to suggest the local name.
